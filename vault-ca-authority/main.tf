...
resource "vault_pki_secret_backend_root_cert" "root_2023" {
  ...
  type        = "internal"
  common_name = var.root_common_name # "example.com"
  ttl         = 315360000
  issuer_name = "root-2023"
}
...
resource "vault_pki_secret_backend_intermediate_cert_request" "csr-request" {
  ...
  type        = "internal"
  common_name = "${var.root_common_name} Intermediate Authority"
}
...
resource "vault_pki_secret_backend_issuer" "intermediate" {
  ...
  issuer_name = "${var.root_common_name}-intermediate"
}
...
resource "vault_pki_secret_backend_role" "intermediate_role" {
  ...
  name             = var.root_common_name # "example-dot-com"
  ttl              = 86400
  max_ttl          = 2592000
  allow_ip_sans    = true
  key_type         = "rsa"
  key_bits         = 4096
  allowed_domains  = [var.root_common_name]
  allow_subdomains = true
...
resource "vault_pki_secret_backend_cert" "example-dot-com" {
  ...
  common_name = "test.${var.root_common_name}"
  ttl         = 3600
  revoke     = true
}
